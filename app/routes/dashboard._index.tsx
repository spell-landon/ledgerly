...
import { FileText, Receipt, BarChart3, Plus, Settings, TrendingUp, TrendingDown, DollarSign, AlertCircle } from "lucide-react";
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const currentMonthExpensesAmount = currentMonthExpenses
    .reduce((sum, exp) => sum + exp.amount, 0);

  const lastMonthExpensesAmount = lastMonthExpenses
    .reduce((sum, exp) => sum + exp.amount, 0);
  ...
  const stats = {
    ...
    incomeChange: lastMonthIncome > 0
      ...
      : 0,
    expensesChange: lastMonthExpensesAmount > 0
  ...
  return json({ stats, recentInvoices, recentExpenses, user: session.user }, { headers });
}
...
export default function DashboardIndex() {
  const { stats, recentInvoices, recentExpenses } = useLoaderData<typeof loader>();
            ...
            <div className="flex items-center gap-1 text-xs">
              {stats.incomeChange >= 0 ? (
                <>
                  ...
                  <span className="text-green-600">+{stats.incomeChange.toFixed(1)}%</span>
                </>
              ) : (
                <>
                  <TrendingDown className="h-3 w-3 text-red-600" />
                  <span className="text-red-600">{stats.incomeChange.toFixed(1)}%</span>
                </>
              )}
            ...
            <div className="flex items-center gap-1 text-xs">
              {stats.expensesChange >= 0 ? (
                <>
                  ...
                  <span className="text-red-600">+{stats.expensesChange.toFixed(1)}%</span>
                </>
              ) : (
                <>
                  <TrendingDown className="h-3 w-3 text-green-600" />
                  <span className="text-green-600">{stats.expensesChange.toFixed(1)}%</span>
                </>
              )}
              <span className="text-muted-foreground">from last month</span>
            </div>
